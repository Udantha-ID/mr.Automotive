...
const GenerateReports = () => {
  ...
  useEffect(() => {
    const fetchEmployeeSalaryData = async () => {
      try {
        ...
        console.log("Error fetching employee salary data:", error);
        setSalaryReportData([]);
      }
    };
    ...
    const fetchEmployeeAttendanceData = async () => {
      try {
        const response = await fetch('http://localhost:3000/EmployeeAttendence');
        const data = await response.json();

        if (Array.isArray(data.data)) {
          setAttendanceReportData(data.data);
        } else {
          setAttendanceReportData([]);
        }
      } catch (error) {
        console.log("Error fetching employee attendance data:", error);
        setAttendanceReportData([]);
      }
    };
  ...
  const handleReportChange = (event) => {
    setSelectedReport(event.target.value); // Update the selected report type
  };
  ...
  const generatePDF = () => {
    ...
    if (companyLogoBase64) {
      ...
      doc.addImage(imgBase64, "PNG", 50, 10, 120, 30);
    } else {
    ...
    // Save the PDF
    doc.save(selectedReport === "salary" ? "salary_report.pdf" : "attendance_report.pdf");
  };
  ...
  const generateCSV = () => {
    ...
    link.setAttribute("download", selectedReport === "salary" ? "salary_report.csv" : "attendance_report.csv");
    link.click();
  };
  ...
  return (
    <div className="p-6 bg-PrimaryColor rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold text-dark mb-4">Employee Reports</h2>

      {/* Dropdown selection */}
      <div className="mb-4">
        <label htmlFor="reportType" className="mr-2">Select Report Type:</label>
        <select id="reportType" value={selectedReport} onChange={handleReportChange} className="p-2 border rounded">
    ...
    >
      ...
      {selectedReport === "salary" ? (
        <motion.div
          ...
          className="mt-6"
        >
          <h3 className="text-xl font-semibold mb-4">Employee Salary Data</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white">
              ...
              <tbody>
                {salaryReportData.map((salary) => (
                  <tr key={salary._id} className="text-center">
                    <td className="py-2 px-4">{salary.employeeName}</td>
                    <td className="py-2 px-4">{salary.NIC}</td>
        ...
        <motion.div
          ...
          className="mt-6"
        >
          <h3 className="text-xl font-semibold mb-4">Employee Attendance Data</h3>
          <div className="overflow-x-auto">
            <table className="min-w-full bg-white">
              <thead>
                <tr className="bg-blue-400 text-white">
                  ...
                  <th className="py-2 px-4">Working Hour</th>
                  <th className="py-2 px-4">Total Hours</th>
                </tr>
              </thead>
              <tbody>
                {attendanceReportData.map((attendance) => (
                  <tr key={attendance._id} className="text-center">
                    <td className="py-2 px-4">{attendance.employeeName}</td>
                    <td className="py-2 px-4">{attendance.NIC}</td>
                    <td className="py-2 px-4">{attendance.date}</td>
                    <td className="py-2 px-4">{attendance.InTime}</td>
                    <td className="py-2 px-4">{attendance.OutTime}</td>
                    <td className="py-2 px-4">{attendance.WorkingHours}</td>
                    <td className="py-2 px-4">{attendance.OTHour}</td>
                  </tr>
                ))}
              </tbody>
            </table>
