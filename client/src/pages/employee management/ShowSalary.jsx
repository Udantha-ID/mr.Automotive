...
const ShowEmployee = () => {
  ...
  // Ensure salary is a number and format to 2 decimal places
  const formatSalary = (salary) => {
    return salary ? parseFloat(salary).toFixed(2) : 'N/A';
  };
  ...
  const downloadSalarySlip = () => {
    ...
    if (companyLogoBase64) {
      ...
      doc.addImage(imgBase64, "PNG", 40, 10, 120, 30);
    } else {
      console.warn("Company logo not found.");
    }

    doc.setFontSize(20);
    // doc.text(`mr.Automotive..`, 60, 50);

    doc.setFontSize(14);
    doc.text(`${Salary.employeeName}'s Salary Slip`, 20, 70);

    doc.setFontSize(12);
    doc.text(`NIC: ${Salary.NIC}`, 20, 90);
    doc.text(`From Date: ${Salary.formDate}`, 20, 100);
    doc.text(`To Date: ${Salary.toDate}`, 20, 110);
    doc.text(`Total OT Hours: ${Salary.totalOtHours}`, 20, 120);
    doc.text(`Total OT Amount: ${Salary.totalOtAmount.toFixed(2)}`, 20, 130);
    doc.text(`Basic Salary: ${formatSalary(Salary.basicSalary.toFixed(2))}`, 20, 140);
    doc.text(`EPF : ${formatSalary(Salary.epfAmount)}`, 20, 150);
    doc.text(`ETF : ${formatSalary(Salary.etfAmount)}`, 20, 160);
        ...
        Salary && (
          <div className="flex flex-col border-2 border-sky-400 rounded-xl w-fit p-4">
            ...
            <div className="my-4">
              ...
              <span>{formatSalary(Salary.basicSalary)}</span>
            </div>

            <div className="my-4">
              <span className="text-xl mr-4 text-gray-500">EPF Amount</span>
              <span>{formatSalary(Salary.epfAmount)}</span>
            </div>

            <div className="my-4">
              <span className="text-xl mr-4 text-gray-500">ETF Amount</span>
              <span>{formatSalary(Salary.etfAmount)}</span>
            </div>

            <div className="my-4">
              <span className="text-xl mr-4 text-gray-500">Total Salary</span>
              <span>{formatSalary(Salary.totalSalary)}</span>
            </div>
