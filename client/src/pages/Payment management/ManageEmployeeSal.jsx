...
const ManageEmployeeSal = () => {
    ...
    const [editEmployee, setEditEmployee] = useState(null); // State for editing
    const [searchStatus, setSearchStatus] = useState(''); // Search filter state
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:3000/EmployeeSalary/${id}`);
            const updatedSalaries = employeeSalaries.filter((sal) => sal._id !== id);
            setEmployeeSalaries(updatedSalaries);
            setFilteredSalaries(updatedSalaries);
        } catch (error) {
            setError('Error deleting employee salary');
        }
    };

    // Edit employee salary
    const handleEdit = (sal) => {
        setEditEmployee(sal); // Set the selected employee for editing
    };

    // Save the updated employee salary
    const handleSave = async (id) => {
        try {
            await axios.put(`http://localhost:3000/EmployeeSalary/${id}`, editEmployee);
            const updatedSalaries = employeeSalaries.map((sal) =>
                sal._id === id ? editEmployee : sal
            );
            setEmployeeSalaries(updatedSalaries);
            setFilteredSalaries(updatedSalaries);
            setEditEmployee(null); // Clear edit mode
        } catch (error) {
            setError('Error updating employee salary');
        }
    };
    ...
    const generateReport = () => {
        ...
        const tableColumn = ['Employee Name', 'NIC', 'From Date', 'To Date', 'OT Hours', 'OT Pay', 'Basic Salary', 'Total Salary', 'Status'];
        const tableRows = filteredSalaries.map((sal) => [
            sal.employeeName,
            sal.NIC,
            sal.formDate,
            sal.toDate,
        ...
        doc.save(`Salary-Payments-Report_${date}.pdf`);
      };
    ...
    return (
        <div className="p-8">
            <h1 className="text-3xl font-bold mb-4">Manage Employee Salaries</h1>

            {/* Search Filter */}
            <div className="mb-4">
                <label htmlFor="searchStatus" className="mr-2">Filter by Status:</label>
                <select
                    id="searchStatus"
                    value={searchStatus}
                ...
                <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                    <thead className="bg-gray-800 text-white">
                        <tr>
                            ...
                            <th className="py-3 px-5 text-left">NIC</th>
                            <th className="py-3 px-5 text-left">From Date</th>
                            <th className="py-3 px-5 text-left">To Date</th>
                            <th className="py-3 px-5 text-left">OT Hours</th>
                            <th className="py-3 px-5 text-left">OT Pay</th>
                    ...
                    <tbody>
                        {filteredSalaries.map((sal) => (
                            <tr key={sal._id} className="border-b">
                                ...
                                <td className="py-3 px-5">{sal.NIC}</td>
...
export default ManageEmployeeSal;
